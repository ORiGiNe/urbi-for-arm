...
# BOOST_MISSING_LIB(LIB-NAME, ERROR-MESSAGE)
# ------------------------------------------
# Called to report that a library (e.g., "python" for Boost.Python) is
# missing.  By default, make it an error.  The user to override this
# to some other action which does not need to halt configure.
AC_DEFUN([BOOST_MISSING_LIB],
[AC_MSG_ERROR([Boost.$1: $2])])


# _BOOST_MISSING_LIB(ERROR-MESSAGE, [ACTION-IF-NOT-FOUND])
# --------------------------------------------------------
# Handle an error.  If ACTION-IF-NOT-FOUND is defined, send the error in
# the logs and invoke it.  Otherwise invoke BOOST_MISSING_LIB.
AC_DEFUN([_BOOST_MISSING_LIB],
[m4_ifval([$2],
          [AC_MSG_NOTICE([Boost.BOOST_Library: $1])
          $2],
          [BOOST_MISSING_LIB([BOOST_Library], [$1])])
])
...
AC_CHECK_HEADER([$1],
  ...
  [_BOOST_MISSING_LIB([cannot find <$1>], [$2])])
CPPFLAGS=$boost_save_CPPFLAGS
AC_LANG_POP([C++])dnl
fi
])# BOOST_FIND_HEADER
...
#                [CXX-PROLOGUE],
#                [$6: ACTION-IF-NOT-FOUND], [$7: ACTION-IF-FOUND])
# -------------------------------------------------------------------------
...
dnl start the for loops).
  # An error message if we can't use the library.
  boost_errmsg=
  AC_COMPILE_IFELSE([],
    [ac_objext=do_not_rm_me_plz],
    [boost_errmsg='cannot compile a test that uses it'])
  if test x"$boost_errmsg" = x; then
...
for boost_rtopt_ in $boost_rtopt '' -d; do
  ...
  do
    ...
    do
      # Don't try directories that do not exist.  They waste time, and
      # may hide system libraries (we might believe -L/non/existing
      # finally found the library, although it just would have worked
      # without the -L).
...
case $Boost_lib in #(
  no) _AC_MSG_LOG_CONFTEST
    test x"$boost_errmsg" = x ||
      boost_errmsg='cannot find link flags for $1'
    _BOOST_MISSING_LIB([$boost_errmsg], [$6])
    ;; #(
  yes) $7
    ;;
esac
